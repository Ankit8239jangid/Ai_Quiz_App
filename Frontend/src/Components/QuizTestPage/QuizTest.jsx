...
function QuizTest() {
  ...
  useEffect(() => {
    if (Selectquizze && Selectquizze.questions) {
      ...
      setTimeLeft(Selectquizze.timeLimit * 60 || 0); // Default to 0 if undefined
    }
  }, [Selectquizze]);
  ...
  const handleSubmitQuiz = useCallback(async () => {
    ...
    try {
      if (isAuthenticated()) {
        const response = await axios.post(`${import.meta.env.VITE_BACKEND_URL}/progress/submit`, {
          quizId: id,
          userAnswers,
        });
      ...
      >
        <div className="flex items-center mb-4">
          ...
          <h1 className="text-2xl font-bold">
            {name || (Selectquizze && Selectquizze.title) || 'Quiz'}
          </h1>
        </div>
